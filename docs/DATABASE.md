...
Indexes: `idx_items_list_id` (on `items.list_id`), `idx_items_target_month` (on `items.target_month`), `idx_lists_type` (on `lists.type`), `idx_lists_house_id` (on `lists.house_id`), `idx_lists_custom_category_id` (on `lists.custom_category_id`), `idx_sessions_user_id`, `idx_house_members_user_id`, `idx_users_oidc_identity` (unique, partial), `idx_price_alerts_item_id`, `idx_price_alerts_status`, `idx_custom_categories_user_id`, `idx_space_shares_user_id` (on `space_shares.shared_with_user_id`), `idx_list_shares_user_id` (on `list_shares.shared_with_user_id`).

`price`/`price_auto`/`image_url`/`target_month`/`due_date`/`is_recurring`/`recurrence_rule`/`recurrence_end_date`/`is_urgent` (on `items`), `is_admin` (on `users`), and `house_id`/`custom_category_id`/`icon` (on `lists`) are not part of their tables' `CREATE TABLE` statements in `schema.sql` — each was added after its table already had a shipped shape, so each is applied via the `ALTER TABLE ... ADD COLUMN` guard in `addColumnIfMissing()` (`internal/db/db.go`), run at startup right after the embedded schema. That function is the concrete implementation of the "guard it in Go via `PRAGMA table_info`" approach described below; any future new column on an existing table should follow the same pattern rather than editing the `CREATE TABLE` in place. `idx_lists_house_id`, `idx_items_target_month`, and `idx_lists_custom_category_id` are likewise created in Go (not in `schema.sql`) since each must run after its `ALTER TABLE` — on a fresh database, `schema.sql` alone creates `lists`/`items` without `house_id`/`target_month`/`custom_category_id` yet. `lists.custom_category_id`/`lists.icon` specifically must also run *after* `migrateListsTypeCheck()` (see the comment at that call site in `internal/db/db.go`): that migration rebuilds the `lists` table by copying a fixed, hard-coded column list, so adding either column before it ran (on a database old enough to still need that rebuild) would have silently dropped the column again. `users`, `sessions`, `house_members`, `price_alerts`, `system_settings`, `custom_categories`, `space_shares`, and `list_shares` are brand-new tables added directly in `schema.sql`, so none of them needed this Go-side guard.

Deleting a row from `houses` cascades to delete all its `lists` (which in turn cascades to delete all their `items` and `list_shares`) and all its `house_members` rows — requiring `foreign_keys=ON` (already set on every connection), and working transitively without any extra code. Deleting a `custom_categories` row cascades to delete its `space_shares` rows (but only detaches, via `ON DELETE SET NULL`, the `lists.custom_category_id` of any list that referenced it — see the `custom_categories` section above). Deleting a `user` cascades to their `sessions`, `house_members`, and their own `custom_categories` (which, per above, further cascades to that category's `space_shares` and detaches any list's `custom_category_id` that referenced it), as well as any `space_shares`/`list_shares` row where they are the recipient (`shared_with_user_id`); houses they owned are **not** deleted (a house survives its owner's account being removed, though it may then be left without an owner — that's an acknowledged edge case, not actively guarded against, since account deletion isn't exposed via the API today).
...
- Widening a `CHECK` constraint on an existing table (e.g. `lists.type`, see above): SQLite has no `ALTER TABLE ... ADD/DROP CONSTRAINT` either, so editing the `CREATE TABLE` statement's `CHECK` clause in `schema.sql` alone only takes effect on a fresh database — an existing deployed database keeps enforcing the old, narrower `CHECK` forever unless something rebuilds the table. `migrateListsTypeCheck()` in `internal/db/db.go` is the concrete pattern to follow for this, and it's worth copying exactly rather than reinventing: create the replacement table under a *temporary* name, copy every row across preserving its id, `DROP TABLE` the *old* table (not rename it away), then `ALTER TABLE ... RENAME TO` the temporary table into the now-free original name — all inside one transaction, with `PRAGMA foreign_keys = OFF` around it (set *outside* the transaction, since SQLite ignores changes to that pragma mid-transaction). The ordering matters and was arrived at by testing, not just reading the docs: renaming the *original* table out of the way first (the seemingly more obvious approach) makes SQLite rewrite every other table's foreign key definitions that pointed at it (e.g. `items.list_id`) to reference the temporary name instead — confirmed empirically, and `PRAGMA foreign_keys = OFF` does *not* suppress this, only `PRAGMA legacy_alter_table` would. Once that temporary table is later dropped, those rewritten definitions are left dangling. Dropping the old table outright never triggers that rewrite (only `RENAME` does), and nothing ever pointed at the temporary table's name, so the untouched `REFERENCES lists(id)` in `items` simply resolves again the moment something named `lists` exists. Any indexes on the table need recreating too, since `DROP TABLE` takes them with it. Verify with `PRAGMA foreign_key_check` before committing rather than trusting the reasoning blindly — `migrateListsTypeCheck()` does this.
