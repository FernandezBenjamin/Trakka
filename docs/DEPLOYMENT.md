...
| `SCRAPE_INTERVAL` | `12h` | How often the target-price background worker (see [docs/API.md](API.md#price-drop-alerts)) re-scrapes every item with an active `alert_on_price_drop` threshold and applies whatever price it finds — distinct from `PRICE_CHECK_INTERVAL_HOURS` above, which drives the separate accept/reject deal-detection scan. Accepts a plain Go duration (`2h`, `30m`) or a whole number of days with a `d` suffix (`1d`), same as `NOTIF_RECURRING_TASK_LEAD_TIME` below. Set to `0` (or negative) to disable the periodic scan entirely. |
...
| `NOTIF_RECURRING_TASK_LEAD_TIME` | `24h` | How long before a recurring item's `due_date` a reminder push is sent (see [docs/API.md](API.md#push-notifications)), unless a specific item overrides it via `recurrence_lead_minutes`. Accepts a plain Go duration (`2h`, `30m`) or a whole number of days with a `d` suffix (`1d`, `3d`). |
| `NOTIF_RECURRING_SCAN_INTERVAL_MINUTES` | `30` | How often the reminder scan itself runs — independent of, and normally much finer-grained than, the lead time above, so a short lead time is actually caught close to on time. `0` (or negative) disables the periodic scan; only takes effect when Web Push is configured at all. |
